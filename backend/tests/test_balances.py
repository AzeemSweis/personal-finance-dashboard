...
class TestBalancesEndpoints:
    ...
    def test_get_balance_overview_with_accounts(self, authenticated_client, sample_account_data):
        ...
        assert data["total_available_balance"] == sample_account_data["available_balance"]
        assert data["currency"] == "USD"
    ...
    def test_get_balance_overview_multiple_accounts(self, authenticated_client, sample_account_data):
        ...
        data = response.json()
        expected_total = account1_data["current_balance"] + account2_data["current_balance"]
        expected_available = account1_data["available_balance"] + account2_data["available_balance"]
        
        assert data["total_balance"] == expected_total
        assert data["total_available_balance"] == expected_available
        assert len(data["accounts"]) == 2

    def test_get_balance_overview_excludes_archived_accounts(self, authenticated_client, sample_account_data):
    ...
    def test_get_balance_snapshots_with_filters(self, authenticated_client):
        ...
        response = client.get(f"/balances/snapshots?start_date={start_date}&end_date={end_date}")
        assert response.status_code == status.HTTP_200_OK

    def test_get_account_balance_snapshots_empty(self, authenticated_client, sample_account_data):
    ...
    def test_get_account_balance_snapshots_wrong_user(self, authenticated_client, sample_account_data, db_session):
    ...
    def test_balance_overview_currency_handling(self, authenticated_client, sample_account_data):
        ...
        usd_account_data = next(acc for acc in data["accounts"] if acc["currency"] == "USD")
        eur_account_data = next(acc for acc in data["accounts"] if acc["currency"] == "EUR")
